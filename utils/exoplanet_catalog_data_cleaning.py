...
def filter_over_neg20_dec(cmplt_catalog_file):
    ...
    filtered_df = cmplt_catalog[cmplt_catalog['dec']
...
def calc_absorbtion_signal(data):
    ...
    gravity = G * data['pl_massj'] * 1.898e27 / \
        (data['pl_radj'] * 69911000.0) ** 2  # kg from m_jup
...
def filter_catalog_no_nan_signal(data_file):
    ...
    exoplanets_no_nan_signal = data[~np.isnan(
        data['signal_percent'])].reset_index(drop=True)
...
if __name__ == '__main__':
    ...
    cmplt_exoplanet_catalog = sync_catalogs(
        to_sync=eu_catalog, sync_wth=arxv_catalog)
    cmplt_exoplanet_catalog.to_csv(
        'data/exoplanet_catalogs/complete_catalog.csv')
    ...
    exoplanets_over_neg20_dec.to_csv(
        'data/exoplanet_catalogs/exoplanets_dec_over_20.csv')

    exoplanets_no_nan_signal = filter_catalog_no_nan_signal(
        'data/exoplanet_catalogs/exoplanets_dec_over_20.csv')
    exoplanets_no_nan_signal.to_csv(
