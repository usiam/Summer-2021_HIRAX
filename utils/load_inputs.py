...
def scale_stellar(so, vmag):
	...
	nphot_expected_0   = calc_nphot(so.filt.dl_l, so.filt.zp_v, vmag)      # what's the integrated V flux supposed to be in photons/m2/s?
...
class fill_data():
	...
	def telluric(self,so):
		...
		tck_tel   = interpolate.splrep(data['Wave/freq'],data['Total'], k=2, s=0)
