...
def plot_vmag_signal_percent_noise_tfac(
    ...
    for j, tfac in enumerate(tfactor):
        model_3sig_noise_percent = nsigma * model_noise_data[f"amplitude_err_tfac{j + 1}"] * 100
        if j % 2:
            marker = '-'
        else:
            marker = '--'
...
def find_detectable_exoplanet_counts(tfactor_ind, exoplanets_data, model_noise_data):
    ...
    model_3sig_percent = 3 * model_noise_data[f'amplitude_err_tfac{tfactor_ind + 1}'] * 100

    tck = interpolate.splrep(model_3sig_percent, model_noise_data['magnitude'], k=3, s=0)
    ...
    target_mags = interpolate.splev(exoplanets_in_limit['signal_percent'], tck, der=0)
    detectables = exoplanets_in_limit[(exoplanets_in_limit['sy_vmag']<target_mags)].reset_index(drop=True)
...
def display_counts(counts, tfactor, savefig=False):
    ...
    df = pd.DataFrame(data_lists, columns=['Configurations'] + throughput_labels)
...
if __name__=='__main__':
    ...
    for i in range(num_configs):
        plot_vmag_signal_percent_noise_tfac(config=f'config{i+1}', savefig=True)
    ...
    for i in range(len(counts)):
        ...
        for j in range(len(tfactor)):
            counts[i].append(find_detectable_exoplanet_counts(tfactor_ind=j, exoplanets_data=exoplanets, model_noise_data=model_noise_data))
